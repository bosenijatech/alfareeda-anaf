...
class _ViewPaySlipPageState extends State<ViewPaySlipPage> {
  ...
  Future<void> _downloadPdf(String url) async {
    try {
      ...
      if (Platform.isAndroid) {
        downloadsDir =
            Directory('/storage/emulated/0/Download/alfareedaanaf/Payslip');
        if (!await downloadsDir.exists()) {
          await downloadsDir.create(recursive: true);
        }
      } else if (Platform.isIOS) {
        downloadsDir = await getApplicationDocumentsDirectory();
        downloadsDir = Directory('${downloadsDir.path}/alfareedaanaf/Payslip');
        if (!await downloadsDir.exists()) {
          await downloadsDir.create(recursive: true);
        }
      }
      ...
      AppUtils.showSingleDialogPopup(
        context,
        "Saved to:\n${"Download/alfareedaanaf/Payslip/"} Folder ",
        "OK",
